...
\addsec{Studieneingangstest - Mathematische Vorkenntnisse}
	\begin{itemize}[label=\mycbox{mydotcolor}]
		\item Andreas Borowski wurde eingeladen um über den Studieneingangstest, der von ihm zum WiSe 2013/14 durchgeführt wurde zu berichten. (Wurde bereits in Bremen angekündigt)
	\end{itemize}

\addsec{IT-Infrastruktur}
	\begin{itemize}[label=\mycbox{mydotcolor}]
		\item Server wurde zum Teil schon aufgesetzt... Freiwillige melden bei unserem IM
...
\addsec{Akkreditierungspool}
	\begin{itemize}[label=\mycbox{mydotcolor}]
		\item 16 Personen im \emph{Programmakkreditierungspool},\\ 5 Personen im \emph{Systemakkreditierungspool}
		\begin{itemize}[label=\color{mydotcolor2}$\pmb\rightarrow$]
			\item Verl"angerung der Entsendung:\quad \emph{Csongor Keuer (TU Berlin), Benjamin Dummer (HU Berlin)}
			\item Auslaufende Mandate \emph{Claudio Michaelis} und \emph{Samuel Greiner}
		\end{itemize}
		\item 33. PVT in Aachen (05.12 - 07.12.2014)
		\item 34. PVT in Kaiserslautern (20.03. - 22.03.2015)
		\item nächstes PVT steht noch nicht fest  
	\end{itemize}
\pagebreak
\begin{center}
	\begin{framed}
		{{\Huge \textsf{\textbf{Wichtig}}} \\~\\ \Large Aktuelle Anmeldeformulare f\"ur den Pool ausf\"ullen und in digitaler Form an die Verwaltung senden!}
	\end{framed}
\end{center}
\pagebreak
...
\addsec{MeTaFa}
	\begin{itemize}[label=\mycbox{mydotcolor}]
		\item Treffen in Aachen (20.03 - 22.03.2015)
		\begin{itemize}[label=\color{mydotcolor2}$\pmb\rightarrow$]
			\item Themen: CHE - Ranking, Vorgehen bei gemeinsamen\\ Resolutionen, Förder-/Trägervereine von BuFaTas 
		\end{itemize}
		\item n\"achstes Treffen wahrscheinlich in Braunschweig \emph{(in Planung)}
	\end{itemize}
\addsec{Kontakt zu anderen BuFaTas}
	\begin{itemize}[label=\mycbox{mydotcolor}]
		\item Sammlung von BuFaTa Terminen im MeTaFa-Wiki 
		\item Zu Gast bei kommenden BuFaTas: Appell an Plenum 
	\end{itemize}
	\textbf{Termine anderer BuFaTas:}
	\begin{itemize}[label=\mycbox{mydotcolor}]
    	\item zeitgleich PsyFaKo in Heidelberg		
		\item Sportwissenschaften in KN (11.06 - 14.06.2015)
	\end{itemize}
...
\section*{Aktuelle Zusammensetzung}
	\begin{itemize}[label=\mycbox{mydotcolor}]
		...
		\item jDPG: \quad \emph{Hejo Kerl (ETH Z"urich) \& \\ Eric Abraham (Uni Jena)}
	\end{itemize}
\section*{Sprecherschaft}
	...
	\end{itemize}
\pagebreak
\section*{Berichte der letzten KFP-Sitzung}
	\begin{itemize}[label=\mycbox{mydotcolor}]
		\item 26.05. - 27.05.15 (Bad Honnef)
	\end{itemize}
	\textbf{Themen:}
%	\begin{multicols}{2}
		\begin{itemize}[label=\mycbox{mydotcolor}]
			\item Berufsorientierung im Bachelor
			\item Gute wissenschaftliche Praxis bei Qualifizierungsarbeiten
			\item Studienatlas Physik
			\item Fortgeschrittenen Praktikum
			\item Habilitationsverfahren (Studie)
		\end{itemize}
%	\end{multicols}
...
\section*{CHE-Hochschulranking}
	...
	\end{itemize}
		...
		\begin{itemize}[label=\color{mydotcolor2}$\pmb\rightarrow$]
			...
			\item Indikatoren wurde zum Teil übernommen, bspw. \emph{Forschungsprofil der Hochschule} statt \emph{Reputationen.} 
		\end{itemize}
		\textbf{Aktuelle Aufgaben:}
		\begin{itemize}[label=\color{mydotcolor2}$\pmb\rightarrow$]
			\item Auswertung und Analyse der diesjährigen Umfrage
		\end{itemize}
\pagebreak
\thispagestyle{empty}
\begin{tikzpicture}[remember picture,overlay]
...
{\color{myfontcolor1}
	\begin{center}
		\vspace{-1.0cm}
		{\Huge \textbf{Fragen? Anregungen? ...}}\vspace{0.5cm}
		\includegraphics[width=.8\textwidth]{Sitzung.jpg}
	\end{center}
}
\end{document}
